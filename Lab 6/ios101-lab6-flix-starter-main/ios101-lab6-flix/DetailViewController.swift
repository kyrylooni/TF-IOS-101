...
//  ios101-lab6-flix
//
//  Created by Kyrylo Onishchenko on 7/13/25.
//
...
class DetailViewController: UIViewController {
    ...
    @IBOutlet weak var backdropImageVio: UIImageView!
    ...
    override func viewDidLoad() {
        ...
        if let voteAverage = movie.voteAverage {
            ...
            voteLable.text = "Vote Average: \(voteAverage)"
        } else {
        ...
        if let releaseDate = movie.releaseDate {
            ...
            releaseDateLable.text = "Release Date: \(releaseDateString)"
        } else {

            // if release date is nil, set release date label text to empty string
            releaseDateLable.text = ""
        ...
        let imageUrl = URL(string: "https://image.tmdb.org/t/p/w500" + backdropPath) {
            ...
            // Use the NukeExtensions library's load image function to (async) fetch and load the image from the image url.
            NukeExtensions.loadImage(with: imageUrl, into: backdropImageVio)
        }
